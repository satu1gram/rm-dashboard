...
import { Copy, Filter, Check, MessageSquare, Heart, Bookmark, Eye, TrendingUp, Lightbulb } from 'lucide-react';
import { useStore } from '../store';
import { Card, Badge, Button } from '../components/ui';
import { PLATFORM_LABEL, PLATFORM_COLOR, STATUS_LABEL, TYPE_LABEL, type Platform, type Content } from '../types';

export default function Konten() {
  const { contents, query } = useStore();
  ...
  const filtered = useMemo(() => {
    ...
    return contents.filter((c) => {
      ...
      return (
        ...
        PLATFORM_LABEL[c.platform].toLowerCase().includes(q)
      );
    });
    ...
    <div className="space-y-3">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-[13px] text-[#6b6b80]">Semua konten siap copy-paste. Klik kartu buat liat detail.</p>
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-3">
        ...
        <div className="xl:col-span-2 space-y-2.5">
          {sorted.map((c) => (
            <Card
              key={c.id}
              className={`p-4 cursor-pointer transition duration-150 hover:border-rm-300 active:scale-[0.995] ${selected?.id === c.id ? 'border-rm-400 ring-2 ring-rm-100' : ''}`}
            >
              <div onClick={() => setSelected(c)}>
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <span className="text-[11px] text-[#76768c]">
                    {c.scheduled_date} • {c.scheduled_time} WIB
                  </span>
                  <span
                    className="ml-auto rounded-full px-2 py-0.5 text-[11px] font-semibold"
                    style={{
                      color: c.status === 'posted' || c.status === 'evaluated' ? '#047857' : c.status === 'ready' ? '#4338ca' : '#b45309',
                ...
                {c.metrics && (
                  <div className="mt-3 flex items-center gap-4 text-[11.5px] text-[#6b6b80]">
          ...
          {selected ? (
            <Card className="p-5">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[11px] text-[#76768c]">{selected.scheduled_time} WIB</span>
              </div>
            ...
            <Card className="p-6 text-center">
              ...
              <p className="text-[12px] text-[#76768c]">Copy-paste siap posting, performa, dan catatan.</p>
            </Card>
          )}
        </div>
      </div>
